...
function CarruselGatos({ selectedRegionId }) { //Función que diseña el carrusel
  const theme = useMantineTheme();
  const mobile = useMediaQuery(`(max-width: ${theme.breakpoints.sm})`);
  //usestate para definir si la consulta a la API está cargando
  const [isLoading, setIsLoading] = useState(true);
  //usestate para definir si existe un error
  ...
  useEffect(() => { //Consulta a la API
    ...
    let link = `https://huachitos.cl/api/animales/tipo/gato`;

    if (selectedRegionId !== 0 && selectedRegionId >= 1 && selectedRegionId <= 16) { //Selecciona endpoint según la selección del filtro
      link = `https://huachitos.cl/api/animales/region/${selectedRegionId}/tipo/gato/estado/adopcion`;
    }
